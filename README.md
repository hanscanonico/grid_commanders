...
`foursquare` is the smallest, 12×12 and the
...
one body every port opens onto. `confluence` is the only board where **every seat fields all three
domains** — two bases, an airport and a port apiece, four rivers running down into one central sea,
and a harbour island of neutral docks nothing walks to. It is the board a 3v1 wants, and asks which
domain the three will neglect.
