...
armour door is a one-cell road gate — the shoulder path is mountain, so boots climb it and nothing
drives it — held by a detachment of four with a city of its own to repair on: your bases and cities
are home, the crown you cannot lose is not, and the shortest march to a match-ending capture is the
one your own infantry make on turn one. Neither home base is within a two-turn march of the crown
it faces, so a pocket falls to an army that walked to it rather than to the base next door. `powder_keg` (14×10) is the roster's smallest economy: an HQ and one base a side,
