...
## finding A0). Every signal below is consumed by `MissionStrip`; `cursor_moved`
## and `power_activated` were deleted because nothing ever connected to them, and
## the first fired once per cursor keypress — unobserved work on the hot path,
## and a fourth autoload implying a decoupling that did not exist. Add the next
## one in the same commit as its first consumer, not before.
