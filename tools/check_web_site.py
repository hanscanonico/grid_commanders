...
`html/head_include` — which is what `make check` runs, no export needed. Given
the directory `make export-web` (or the Docker image) built, it additionally
opens `play/index.html` and holds the real exported game page to the same bar.

Every assertion is about the *served* bytes, never about how they were authored:
a title, a description short enough to survive a result page, a canonical on the
apex host, the three Open Graph tags a link preview needs, a JSON-LD block that
parses, a link into the game, a robots file that names the sitemap, a sitemap
listing exactly the two pages, and an OG image that really is 1200x630. The
"Advance Wars" name is held to body copy and the meta description — never the
title, a heading or og:title (CLAUDE.md's legal note).
"""
...
def check_game_head(html: str, where: str) -> None:
	...
	if not description:
		fail("%s: the game page has no meta description" % where)
	elif len(description) > DESCRIPTION_MAX:
		fail("%s: the game page's description is %d chars (max %d)" % (where, len(description), DESCRIPTION_MAX))
	href = canonical(html)
	if href != SITE + "/play/":
...
def check_landing(html: str, where: str) -> None:
	...
	if not description:
		...
		fail("%s: the description is %d chars (max %d)" % (where, len(description), DESCRIPTION_MAX))
	...
	for block in blocks:
		...
		if data.get("@type") != "VideoGame":
			fail("%s: the JSON-LD block is a %r, expected a VideoGame" % (where, data.get("@type")))
	...
	named = [title.group(1) if title else ""] + headings + [meta(html, "property", "og:title") or ""]
	for text in named:
		if FORBIDDEN in text.lower():
			fail("%s: %r names Advance Wars — body copy and the description only" % (where, text.strip()))
