...
# The off-desktop capture renderer's engine version and image tag (COM-279),
# shared with tools/godot_gui.sh so a bump is stated once.
include tools/capture/image.env
...
verify: check capture-test lint format-check test determinism

# What tools/godot_gui.sh does with a launch, on fakes only: no Docker, no
# engine, nothing rendered. In the gate because the launcher is what every
# capture goes through.
capture-test:
	tools/test_godot_gui.sh

# Builds the capture image an agent's screenshots render inside — never built
# implicitly, since a capture that quietly spent ten minutes fetching Debian
# reads as a hung capture. Needs Docker with a linux/arm64 daemon; without it
# every capture falls back to the windowed path and says so.
capture-image:
	docker build --platform linux/arm64 \
		--build-arg GODOT_VERSION=$(GODOT_CAPTURE_VERSION) \
		-t $(GODOT_CAPTURE_IMAGE) tools/capture
...
.PHONY: run hotseat test verify smoke check determinism capture-test capture-image lint format format-check tiles \
	atlases \
