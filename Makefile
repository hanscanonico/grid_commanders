...
# The unit and terrain atlases are generated by the sibling sprite_generator
# repo (deterministic voxel pipeline, no third-party art). Point SPRITEGEN at
...
# `atlases` regenerates both sheets in the sprite_generator checkout and
# installs them (plus the per-cell reference sprites) here; `ui-art` draws the
# overlay, cursor and icon, the art that has no home in that repo.
...
atlases:
	@test -x "$(SPRITEGEN)/.venv/bin/python" || { \
		echo "$@: sprite_generator not found at $(SPRITEGEN)" >&2; \
		echo "$@: clone it beside this repo (see README), or pass SPRITEGEN=<path>" >&2; \
		exit 1; }
	"$(SPRITEGEN)/.venv/bin/python" "$(SPRITEGEN)/sprite_generator.py" \
