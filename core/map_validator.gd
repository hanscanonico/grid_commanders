...
##
## Every string it returns is addressed to the author, not to a log: it names the
## cell and says what the board would do wrong, because "invalid map" is not an
## actionable failure message. An empty array means the board plays.
##
...
static func errors(map: MapData) -> Array[String]:
	...
	_append(found, _size_error(map))
	found.append_array(_seat_errors(map))
	_append(found, _property_count_error(map))
	_append(found, _spare_hq_error(map))
	_append(found, _hq_connection_error(map))
	_append(found, _unit_on_property_error(map))
	_append(found, _dock_error(map))
	return found
...
static func draft_errors(doc: MapDocument, db: TerrainDB) -> Array[String]:
	...
	if map == null:
		return ["This board cannot be saved as a map file — see the log for what it refused."]
	return errors(map)


static func _append(found: Array[String], error: String) -> void:
	if error != "":
		found.append(error)


static func _size_error(map: MapData) -> String:
	var size := map.size()
...
static func _seat_errors(map: MapData) -> Array[String]:
	var found: Array[String] = []
	for team in map.teams():
		...
		if factories == 0:
			found.append(
				"Seat %d owns nothing that builds units, so it can never spend its income." % team
			)
	return found
...
static func _property_count_error(map: MapData) -> String:
	var seats := map.player_count()
	var properties := map.property_cells().size()
	if properties >= seats:
		return ""
	return "The board holds %d properties; %d seats need at least %d." % [properties, seats, seats]
...
static func _spare_hq_error(map: MapData) -> String:
	for cell in map.property_cells():
		if map.terrain_at(cell).is_headquarters and map.owner_at(cell) == MapData.NEUTRAL:
			return "The headquarters at %s belongs to nobody; an unowned one fells no army." % cell
	return ""
...
static func _hq_connection_error(map: MapData) -> String:
	var hqs := _headquarters(map)
	if hqs.size() < 2:
		return ""  # the per-seat count owns this case
...
static func _unit_on_property_error(map: MapData) -> String:
...
static func _dock_error(map: MapData) -> String:
