...
`battle-animations-plan.html`, the zoom ladder and the animation milestone, and
`mobile-builds-plan.html`.
...
- `battle-animations-plan.html` — the combat cut-in BA1–BA4, all shipped. D1: **the cut-in replays
  ...
  `BattleStyleDB.for_weapon` and never re-decides, `attacker_indirect`, and
  `attacker_cover_stars` / `defender_cover_stars`. The last three
  ...
  cover pair arrived the same way: the plate was printing the tile's own `defense_stars`, so
  an aircraft over a mountain was drawn four stars and then took damage as if it had none.
  `CombatResolver` stays the one authority on how much cover a cell gives — the two fields are
  its answer, taken at the Engagement's cells when the shot resolved. The
