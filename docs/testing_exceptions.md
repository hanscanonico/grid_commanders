...
| `BattlePerspective.can_see_funds` | `test_battle_perspective_funds.gd` | the adapter is a `RefCounted` over a `GameState`, and this reading takes neither a cell nor a sprite: who may read a seat's bank is the only thing keeping the commander sheet's economy row off an enemy treasury, and a captured frame cannot show a rule that was asked the wrong way round |
