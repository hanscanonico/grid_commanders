...
| `TurnOpening`, `BattlePerspective.reportable_losses` | `test_turn_opening.gd` | `lines` is a pure function of a `GameState`, a `SideIdentity`, a viewer and what the turn's opening took, and the viewer rule under it is a read of the same state — so which words a turn opens with, and which of its losses this seat may be told about, are checked without a board on screen. The claim that pays for the row is the one a fog match hides: a lone plane of the viewer's own side runs dry where nothing is left to watch it, and the seat is still told |
...
| `UiTheme`, `UiMarks`, `DisabledPalette` | `test_ui_theme_box.gd`, `test_ui_theme_font.gd`, `test_ui_theme_sizes.gd`, `test_ui_marks.gd`, `test_disabled_palette.gd` | fonts, style boxes and the drained palette come back from static calls as `Resource`s and `Color`s, so the metrics a layout depends on and the contrast a greyed row keeps are read without building a `Control`. `test_ui_theme_sizes.gd` reads the size constants the same way: the ladder never steps down, body and tip are one size, and the only tokens under the pixel face's grid are the two named exceptions. `test_ui_marks.gd` is the same shape one layer down: the six marks neither pixel face draws are pixel masks baked into a `FontFile`, so the sheet and the fallback wiring behind every served face are read off the resource — which is the check a captured frame cannot make, the desktop having a system font to fall through to and a browser none |
| `CommanderVisuals`, `SideIdentity` | `test_side_identity.gd`, `test_side_identity_roster.gd`, `test_side_identity_colorblind.gd`, `test_commander_face.gd`, `test_commander_portraits.gd` | the single authority for a side's presentation: a portrait, a faction theme and an atlas row, resolved once from the match's commander picks with no `Node` and no scene path. `test_commander_face.gd` and `test_commander_portraits.gd` read `face_for` and `portrait_for` on the same terms, a baked bust and the face chip. The chip is that same drawing repainted on its own coarser grid, not a cut out of the bust, and the two share no bytes. `test_commander_portraits.gd` also pins the two pure functions the pixel art rests on: `art_scale`'s whole-number ladder, and `ART_FILTER` being the nearest one. The *measurements* live in `generators/portraits/tests/test_face_region.py` instead, per bust: that `FACE_REGION` clears every jaw, and that each chip texel carries the tone dominating the 3x3 block of its own bust it stands for, by a margin no other general's bust comes near. `test_side_identity_colorblind.gd` measures the same five `theme_for` hues once red and green have collapsed onto one axis |
