...
def infantry() -> Model:
    ...
    m.set(7, 6, 5, "skin")               # trigger hand
    # rifle held at chest height along the right side, barrel toward +y
    m.set(7, 5, 5, "wood")               # stock
    m.box(7, 7, 6, 10, 5, 5, "gunmetal")
    m.set(7, 11, 5, "gunmetal_dk")
...
def recon() -> Model:
    ...
    for x in (0, 8):
        ...
        _tire(m, x, 10)
    # hull bed
    m.box(1, 8, 0, 13, 2, 3, "body")
    # sloped hood toward the front
    m.box(1, 8, 14, 14, 2, 2, "body")
    m.box(2, 7, 14, 15, 2, 2, "body_lt")
    m.box(2, 7, 12, 13, 3, 3, "body_lt")  # hood top
    # bumper and headlights
    m.box(1, 8, 15, 15, 2, 2, "body_dk")
    m.set(2, 15, 2, "amber")
    m.set(7, 15, 2, "amber")
    # cabin with windshield and side glass
    m.box(2, 7, 3, 9, 4, 5, "body")
    m.box(3, 6, 9, 9, 4, 5, "glass")
    m.box(7, 7, 4, 7, 4, 4, "glass_dk")
    m.box(2, 7, 3, 3, 4, 5, "body_dk")   # rear cabin plate
...
def tank() -> Model:
    ...
    _track(m, 9, 11, 0, 13)
    # hull
    m.box(0, 11, 1, 12, 2, 3, "body")
    m.box(1, 10, 13, 13, 2, 3, "body")
    m.box(2, 9, 14, 14, 2, 2, "body_lt")   # glacis lip
    m.box(2, 9, 13, 13, 3, 3, "body_lt")   # glacis top
    # rear deck vents and exhausts
    m.box(2, 9, 1, 1, 3, 3, "body_dk")
    m.box(2, 9, 3, 3, 3, 3, "body_dk")
    m.box(0, 1, 0, 0, 2, 3, "gunmetal_dk")
    # turret with dome and cupola
    m.box(3, 8, 4, 9, 4, 5, "body")
    m.box(4, 7, 5, 8, 6, 6, "body")
    m.box(4, 5, 5, 6, 7, 7, "body_dk")     # commander cupola
    m.set(7, 8, 6, "body_lt")              # loader hatch glint
    m.box(3, 8, 4, 4, 6, 6, "body_dk")     # stowage bustle
    # mantlet and gun at turret height
    m.box(4, 7, 10, 10, 4, 5, "body_dk")
    m.box(5, 6, 10, 16, 5, 5, "gunmetal")
    m.box(5, 6, 13, 13, 5, 5, "gunmetal_dk")  # bore evacuator
    m.box(4, 7, 16, 16, 5, 5, "gunmetal_dk")  # muzzle brake
    m.box(5, 6, 17, 17, 5, 5, "bore")
    # antenna
    m.box(3, 3, 4, 4, 7, 9, "steel")
    return m
...
def md_tank() -> Model:
    ...
    m.box(0, 12, 1, 14, 3, 4, "body")
    m.box(10, 12, 2, 13, 3, 3, "body_dk")
    m.box(0, 2, 2, 13, 3, 3, "body_dk")
    # stepped heavy glacis
    m.box(1, 11, 15, 15, 3, 4, "body")
    m.box(2, 10, 16, 16, 3, 3, "body_lt")
    m.box(2, 10, 15, 15, 4, 4, "body_lt")
    # rear engine deck and twin exhausts
    m.box(2, 10, 1, 2, 4, 4, "body_dk")
    m.box(1, 2, 0, 0, 3, 4, "gunmetal_dk")
    m.box(10, 11, 0, 0, 3, 4, "gunmetal_dk")
    # big turret, two tiers
    m.box(3, 9, 4, 11, 5, 6, "body")
    m.box(4, 8, 5, 10, 7, 7, "body")
    m.box(3, 9, 4, 4, 7, 7, "body_dk")     # bustle rack
    m.box(4, 5, 5, 6, 8, 8, "body_dk")     # cupola
    m.box(7, 8, 6, 7, 8, 8, "body_lt")     # hatch
    # wide mantlet, long gun with thermal-sleeve rings
    m.box(4, 8, 12, 12, 5, 7, "body_dk")
    m.box(5, 7, 13, 19, 6, 6, "gunmetal")
    m.box(5, 7, 15, 15, 6, 6, "gunmetal_dk")
    m.box(5, 7, 17, 17, 6, 6, "gunmetal_dk")
    m.box(4, 8, 19, 19, 6, 6, "gunmetal_dk")  # muzzle brake
    m.box(5, 7, 20, 20, 6, 6, "bore")
    # antenna
    m.box(3, 3, 5, 5, 8, 9, "steel")
    return m
...
def anti_air() -> Model:
    ...
    _track(m, 8, 10, 0, 11)
    # low hull
    m.box(0, 10, 1, 11, 2, 3, "body")
    m.box(1, 9, 12, 12, 2, 3, "body_lt")
    m.box(2, 8, 1, 1, 3, 3, "body_dk")
    # rotating battery box
    m.box(2, 8, 3, 8, 4, 6, "body")
    m.box(2, 8, 3, 8, 4, 4, "body_dk")
    m.box(3, 7, 3, 3, 5, 6, "body_dk")   # ammo feed
    # twin flak tubes, well separated, climbing toward +y
    for x in (3, 7):
        m.box(x, x, 9, 10, 6, 6, "gunmetal")
        m.box(x, x, 11, 12, 7, 7, "gunmetal")
        m.set(x, 13, 8, "gunmetal_dk")
        m.set(x, 14, 8, "bore")
    # ranging light on the battery roof
    m.set(5, 8, 7, "amber")
    # search radar panel on the rear corner
    m.box(8, 8, 4, 4, 7, 8, "steel")
    m.box(7, 8, 3, 4, 9, 9, "steel")
...
def artillery() -> Model:
    ...
    _track(m, 0, 2, 0, 12)
    _track(m, 8, 10, 0, 12)
    m.box(0, 10, 1, 12, 2, 3, "body")
    m.box(1, 9, 13, 13, 2, 2, "body_lt")
    # casemate rising toward the rear
    m.box(1, 9, 2, 8, 4, 5, "body")
    m.box(2, 8, 2, 6, 6, 6, "body")
    m.box(1, 9, 2, 2, 4, 6, "body_dk")   # rear plate
    m.box(2, 8, 8, 8, 4, 5, "body_dk")   # front casemate slope
    m.set(3, 4, 7, "body_dk")            # top hatch
    m.set(4, 4, 7, "body_dk")
    # howitzer: fat tube climbing at ~45 degrees toward +y
    ...
    m.box(3, 7, 13, 13, 11, 11, "gunmetal_dk")  # double-baffle muzzle brake
    m.box(4, 6, 14, 14, 11, 11, "bore")
...
def rockets() -> Model:
    ...
    for y in (1, 6, 11):
        ...
        _tire(m, 8, y)
    # chassis bed
    m.box(1, 8, 0, 14, 2, 3, "body")
    # cab at the front with windshield
    m.box(2, 7, 10, 14, 4, 5, "body")
    m.box(3, 6, 14, 14, 4, 5, "glass")
    m.box(7, 7, 11, 13, 4, 4, "glass_dk")
    m.box(2, 7, 15, 15, 2, 3, "body_dk")  # bumper
    m.set(3, 15, 3, "amber")
    m.set(6, 15, 3, "amber")
    ...
    for k in range(3):
        ...
        for x in (2, 4, 6):
            m.set(x, y1, 6 + k, "bore")
    m.box(1, 8, 7, 8, 4, 4, "body_dk")     # pod underside toward the cab
...
def apc() -> Model:
    ...
    _track(m, 8, 10, 0, 12)
    # tall box hull
    m.box(0, 10, 1, 12, 2, 5, "body")
    m.box(1, 9, 13, 13, 2, 4, "body")
    m.box(2, 8, 13, 13, 5, 5, "body_lt")   # nose slope
    m.box(2, 8, 14, 14, 2, 3, "body_lt")
    # driver visor slit and headlights
    m.box(3, 5, 14, 14, 4, 4, "glass_dk")
    m.set(2, 14, 4, "amber")
    m.set(8, 14, 4, "amber")
    # roof: troop hatch and low cargo rail with a crate
    m.box(1, 9, 2, 12, 6, 6, "body")
    m.box(3, 7, 8, 11, 6, 6, "body_dk")
    m.box(2, 4, 3, 5, 7, 7, "wood")
    m.box(8, 8, 2, 6, 7, 7, "body_dk")     # rail
    # rear troop door
    m.box(3, 7, 0, 0, 2, 5, "body_dk")
...
def missiles() -> Model:
    ...
    m.box(2, 7, 15, 15, 2, 3, "body_dk")
    m.set(3, 15, 3, "amber")
    m.set(6, 15, 3, "amber")
    # raked launch rack over the bed
    m.box(1, 8, 1, 3, 4, 5, "body_dk")
    m.box(1, 8, 4, 6, 5, 6, "body_dk")
    m.box(1, 8, 7, 8, 6, 7, "body_dk")
...
def fighter() -> Model:
    ...
    m = Model()
    # fuselage, nose toward +y
    m.box(4, 5, 0, 17, 3, 4, "body")
    m.box(4, 5, 16, 18, 3, 3, "body")
    m.set(4, 19, 3, "body_dk")           # radome tip
    m.set(5, 19, 3, "body_dk")
    # canopy
    m.box(4, 5, 11, 13, 5, 5, "glass")
    m.set(4, 10, 5, "glass_dk")
    m.set(5, 10, 5, "glass_dk")
    # air intakes
    m.box(3, 3, 8, 11, 3, 4, "body_dk")
    m.box(6, 6, 8, 11, 3, 4, "body_dk")
    # swept wings (trailing back as they leave the fuselage)
    for i in range(1, 7):
        wy = 9 - i
...
def bomber() -> Model:
    ...
    m = Model()
    # deep fuselage
    m.box(4, 7, 0, 19, 3, 5, "body")
    m.box(4, 7, 18, 19, 3, 4, "body")
    m.box(5, 6, 20, 20, 3, 4, "body_lt")   # nose cap
    # flight-deck glazing
    m.box(4, 7, 16, 17, 5, 5, "glass")
    # straight wings with slight rearward rake
    for i in range(1, 9):
        wy = 10 - (i + 1) // 2
    ...
    m.box(7, 7, 6, 12, 3, 3, "body_dk")
    # tailplane and tall fin
...
def t_copter() -> Model:
    ...
    m.box(6, 6, 6, 9, 4, 5, "body_dk")
    m.box(6, 6, 2, 15, 3, 3, "body_dk")
    # rear loading ramp
    m.box(3, 6, 1, 1, 3, 5, "body_dk")
    # fixed wheels
...
def battleship() -> Model:
    ...
    m = Model()
    # hull with tapered bow (+y) and stern; dark waterline
    m.box(1, 6, 2, 20, 0, 1, "body")
    m.box(2, 5, 21, 22, 0, 1, "body")
    m.box(3, 4, 23, 23, 0, 1, "body")
    m.box(2, 5, 0, 1, 0, 1, "body")
    m.box(1, 6, 2, 20, 0, 0, "body_dk")
    m.box(2, 5, 21, 22, 0, 0, "body_dk")
    m.box(2, 5, 0, 1, 0, 0, "body_dk")
    # deck stays the faction color, with a walkway line down the centre
...
def cruiser() -> Model:
    ...
    m = Model()
    # hull with dark waterline
    m.box(1, 6, 2, 17, 0, 1, "body")
    m.box(2, 5, 18, 19, 0, 1, "body")
    m.box(3, 4, 20, 20, 0, 1, "body")
    m.box(2, 5, 0, 1, 0, 1, "body")
    m.box(1, 6, 2, 17, 0, 0, "body_dk")
    m.box(2, 5, 18, 19, 0, 0, "body_dk")
    m.box(2, 5, 0, 1, 0, 0, "body_dk")
    ...
    # blocky bridge with glazing facing the bow
    m.box(2, 5, 8, 12, 3, 4, "body")
    m.box(3, 4, 12, 12, 4, 4, "glass_dk")
    m.box(3, 4, 9, 11, 5, 5, "body")
    m.box(3, 4, 11, 11, 5, 5, "glass_dk")
    m.box(3, 3, 10, 10, 6, 7, "steel")     # mast
...
def sub() -> Model:
    ...
    m = Model()
    # cigar hull, rounded by stepped tapers
    m.box(2, 5, 2, 18, 1, 2, "body")
    m.box(3, 4, 19, 20, 1, 2, "body")
    m.box(3, 4, 21, 21, 1, 1, "body")
    m.box(3, 4, 0, 1, 1, 2, "body")
    m.box(2, 5, 4, 16, 3, 3, "body")      # hull crown
    m.box(3, 4, 17, 18, 3, 3, "body")
    # dark anti-fouling waterline
    m.box(2, 5, 2, 18, 0, 0, "body_dk")
    m.box(3, 4, 0, 1, 0, 0, "body_dk")
    m.box(3, 4, 19, 21, 0, 0, "body_dk")
...
def lander() -> Model:
    ...
    m = Model()
    # wide flat hull
    m.box(0, 8, 1, 16, 0, 1, "body")
    m.box(1, 7, 17, 17, 0, 1, "body")
    # gunwales around the cargo well
    m.box(0, 8, 1, 16, 2, 2, "body")
    m.clear(1, 7, 2, 15, 2, 2)
    # cargo well floor with tie-down lane
    m.box(1, 7, 2, 15, 1, 1, "deck_dk")
    m.box(3, 5, 3, 14, 1, 1, "deck")
    m.box(4, 4, 3, 14, 1, 1, "body_dk")
    # blunt bow ramp, raised for sea travel
    m.box(1, 7, 18, 18, 0, 3, "body_lt")
    m.box(1, 7, 18, 18, 4, 4, "body_dk")   # ramp lip
    m.box(2, 6, 18, 18, 2, 3, "body")      # ramp ribs
    # aft wheelhouse
    m.box(2, 6, 1, 4, 2, 4, "body")
    m.box(3, 5, 4, 4, 4, 4, "glass_dk")
    m.box(3, 5, 2, 3, 5, 5, "body_dk")
