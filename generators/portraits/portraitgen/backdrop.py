...
# a tone off the ramp, never an alpha wash over the field: a wash was what the
# retired bake's soft edges hid, and at sixteen tones it is a colour the bust does
# not have. The field is the shadow rung — a band under the coat that stands in
# front of it on every army, Iron included — and the two treatments step DOWN from
# it, so a window can only ever be darker than the general in it.
FIELD_SLOT = S_SHADOW
