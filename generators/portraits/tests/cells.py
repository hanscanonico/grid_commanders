"""How the suites read a raster back, and the working canvas they paint one on.
...
rather than three times over. `tally` is the one place a suite turns an image
into counted colours, and the one place the `getcolors` cap being hit is an
error rather than a `None` that reads as "no colours".
...
def _tally(canvas: Canvas) -> list[tuple[int, tuple[int, ...]]]:
    """Every colour on the canvas and how much of it there is."""
    return tally(canvas.image)
