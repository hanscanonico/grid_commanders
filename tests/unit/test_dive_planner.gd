extends GutTest
## The dive as the computer plays it: when a boat goes under, where it surfaces
## the decision from, and what it counts as a threat worth the fuel.
##
## The sibling of tests/unit/test_dive.gd, which owns the submarine's own rules,
## and of tests/unit/test_dive_detection.gd, which owns what can still see one.
