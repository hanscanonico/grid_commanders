...
func test_capture_cell_is_met_only_while_the_side_holds_the_ground() -> void:
	...
	assert_false(objective.is_met(state, 1), "team 2 holds it at the opening")
	state.set_owner(Vector2i(1, 0), 1)
	assert_true(objective.is_met(state, 1), "and team 1 has taken it")
...
func test_capture_cell_counts_an_allys_capture_as_ours() -> void:
	...
	var objective := CaptureCellObjective.new()
	objective.cell = Vector2i(1, 0)
	assert_true(objective.is_met(state, 1), "our side holds it, whichever of us captured it")
...
func test_capture_cell_is_never_met_by_neutral_ground() -> void:
	...
	var objective := CaptureCellObjective.new()
	objective.cell = Vector2i(1, 0)
	assert_false(objective.is_met(state, 1), "team 0 stands with nobody")
...
func test_own_properties_counts_the_whole_side() -> void:
	...
	objective.count = 3
	assert_true(objective.is_met(state, 1), "one of ours plus two of our ally's")
...
func test_own_properties_counts_only_our_team_when_we_stand_alone() -> void:
	...
	var objective := OwnPropertiesObjective.new()
	objective.count = 3
	assert_false(objective.is_met(state, 1), "a free-for-all counts team 1's one city")
	objective.count = 1
	assert_true(objective.is_met(state, 1))
...
func test_own_properties_can_ask_for_one_kind_of_ground() -> void:
	...
	assert_false(objective.is_met(state, 1), "team 1 holds a city, not the HQ")
...
func test_survive_until_day_reads_the_calendar_only() -> void:
	...
	objective.day = 4
	assert_false(objective.is_met(state, 1))
	state.day = 4
	assert_true(objective.is_met(state, 1), "the start of day 4 is reaching day 4")
...
func test_day_deadline_fires_the_day_after_the_last_playable_one() -> void:
	...
	state.day = 6
	assert_false(objective.is_met(state, 1), "day 6 is still the player's to use")
	state.day = 7
	assert_true(objective.is_met(state, 1), "and day 7 is the deadline having passed")
